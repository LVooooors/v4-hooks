...
contract LvrShieldScript is Script {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, permissions, type(LvrShield).creationCode, abi.encode(address(manager)));
        ...
        vm.startBroadcast();
        testLifecycle(manager, address(lvrShield), lpRouter, swapRouter);
        vm.stopBroadcast();
    }
